...
async def verify_token(token: str):
    async with httpx.AsyncClient() as client:
        response = await client.post(
            f"{os.environ.get('PRIVATE_PB_URL')}/api/collections/users/auth-refresh",
...
async def generate(data: GenerateRequest, request: Request):
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            await client.post(
                f"{os.environ.get('PRIVATE_PB_URL')}/api/collections/messages/records",
                json={
            ...
            response = await client.get(
                f"{os.environ.get('PRIVATE_PB_URL')}/api/collections/conversations/records/{data.conversationId}",
                params={
                    "sort": "created",
